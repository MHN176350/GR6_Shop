 ...
 * @author MILAMILO
 */
@WebServlet(name = "ADBChangePasswordControl", urlPatterns = {"/adbchangepassword"})
public class ADBChangePasswordControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String re_password = request.getParameter("password2");
            ADBAccountDAO adbdao = new ADBAccountDAO();
            ADBAccount adbacc = (ADBAccount) session.getAttribute("ADBUSER");
            String passError;
            String userResponse = request.getParameter("g-recaptcha-response");
            if (userResponse.equals("")) {
                request.setAttribute("message", "You must verify captcha!");
                request.getRequestDispatcher("adbadminprofile.jsp").forward(request, response);
            } else if (adbdao.getPasswordAttempts(adbacc.getUsername()) == 3) {
                session.invalidate();
                request.setAttribute("failed", "You try many times. Your account is locked! Redirecting to Login Page.... ");
                request.getRequestDispatcher("adbadminprofile.jsp").forward(request, response);
            } else {
                if (!password.equals(re_password)) {
                    request.setAttribute("message", "Re-password is not match with New Password!");
                    request.getRequestDispatcher("adbadminprofile.jsp").forward(request, response);
                } else if (!old_password.equals(adbacc.getPassword())) {
                    adbdao.updatePasswordAttempts(adbacc.getUsername());
                    ...
                    adbdao.changePassword(adbacc.getUsername(), re_password);
                    adbdao.resetPasswordAttempts(adbacc.getUsername());
