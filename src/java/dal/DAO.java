...
public class DAO extends DBContext {
    ...
    public void addB(String title, String cont, int a_id, int cate, String thumb) {
        try {
            ...
            st.setInt(4, cate);
                ...
                com = "Insert into blog(title, main, author_id, b_cate,thumbnail) values(?,?,?,?,?)";
                PreparedStatement st = connection.prepareStatement(com);
            st.setString(1, title);
            st.setString(2, cont);
            st.setInt(3, a_id);
   ...
   public List<Blog> getBlist(int cate){
        List<Blog> list =new ArrayList<>();
        try {
            String com="select * from blog b left join b_cate c on b.b_cate=c.c_id ";
           
            if(cate!=0){
                com+=" where b_cate=" +cate;
            }
            PreparedStatement st=connection.prepareStatement(com);
            ResultSet rs=st.executeQuery();
            while(rs.next()){
                Blog b=new Blog(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getString(9),rs.getString(7),rs.getString(5));
                list.add(b);
            }
        } catch (SQLException ex) {
            Logger.getLogger(DAO.class.getName()).log(Level.SEVERE, null, ex);
